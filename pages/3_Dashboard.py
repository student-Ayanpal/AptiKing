...
/* Specifically target the link text color again if needed (often redundant) */
    ...
    if 'startup_check_done' not in st.session_state:
        ...
        if isinstance(stored_login_data, dict) and stored_login_data.get('value') is True:
            ...
            if user_id is not None and username is not None: logged_in = True
                ...
                logged_in = False
                localS_check.setItem('logged_in', False, key="clear_login_inconsistent_dash")
                localS_check.setItem('user_id', None, key="clear_userid_inconsistent_dash")
                localS_check.setItem('username', None, key="clear_username_inconsistent_dash")
             ...
             if stored_login_data is True:
                 ...
                 if user_id is not None and username is not None:
                     localS_check.setItem('logged_in', True, key="fix_login_dash_func")
                     localS_check.setItem('user_id', user_id, key="fix_userid_dash_func")
                     localS_check.setItem('username', username, key="fix_username_dash_func")
                 else: # Invalid details from old format
                      logged_in = False
                      localS_check.setItem('logged_in', False, key="clear_login_old_invalid_dash")
                      localS_check.setItem('user_id', None, key="clear_userid_old_invalid_dash")
                      localS_check.setItem('username', None, key="clear_username_old_invalid_dash")
             else: # Old format was False
                  logged_in = False
                  localS_check.setItem('logged_in', False, key="clear_login_old_false_dash")
                  localS_check.setItem('user_id', None, key="clear_userid_old_false_dash")
                  localS_check.setItem('username', None, key="clear_username_old_false_dash")
        # --- End Stricter Check ---
...
if 'logged_in' in st.session_state and st.session_state.logged_in:
    ...
    if st.sidebar.button("Logout", key="logout_dash_sidebar"):
        ...
        localS_logout.setItem('logged_in', False, key="logout_set_login_status_dash") # Unique key
        localS_logout.setItem('user_id', None, key="logout_set_userid_none_dash")       # Unique key, use None
        localS_logout.setItem('username', None, key="logout_set_username_none_dash")   # Unique key, use None
...
if 'username' in st.session_state:
    st.title(f"Welcome to your Dashboard, {st.session_state.username}! 👋")
     ...
     # Don't show warning here as the security check should have stopped execution if login failed
...
with col2:
    st.image("images/progress.png")
    
    st.markdown(f"""
    <a href="Your_Progress" target="_self" class="dashboard-button-link">
        View Your Progress 📊
    </a>
    """, unsafe_allow_html=True)
...
with col3:
    st.image("images/study_materials.png")
    
    st.markdown(f"""
    <a href="Study_Materials" target="_self" class="dashboard-button-link">
        Get Study Materials 📚
    </a>
    """, unsafe_allow_html=True)
...
with col4:
    st.image("images/doubt_solver.png")

    st.markdown(f"""
    <a href="Doubt_Solver" target="_self" class="dashboard-button-link">
        Ask a Doubt ❓
    </a>
    """, unsafe_allow_html=True)