...
# DO NOT import LocalStorage here globally anymore
import time # Import time
# DO NOT import switch_page

# --- Page settings (MUST be first Streamlit command) ---
st.set_page_config(page_title="AptiKing", page_icon="👑", layout="wide")

# --- Inject CSS for Button Styling & Info Container ---
st.markdown("""
<style>
/* --- NEW: Force remove top padding/margin from main container --- */
div[data-testid="stVerticalBlock"] > [style*="flex-direction: column;"] > [data-testid="stVerticalBlock"]:first-child {
    /* This targets the very first container block on the page (holding the title) */
...
.dashboard-button-link {
    display: block; background-color: #FF4B4B; color: white !important; padding: 0.75rem 1rem;
    border-radius: 0.5rem; text-align: center; text-decoration: none !important;
    transition: background-color 0.2s ease; border: none; margin-top: 0.25rem;
    text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.7);
}
.dashboard-button-link:hover { background-color: #DC3545; color: white !important; text-decoration: none !important; }

/* Style for the info container - now targets the second element block */
div[data-testid="stVerticalBlock"] > [style*="flex-direction: column;"] > [data-testid="stVerticalBlock"]:nth-child(2) {
    border: 1px solid #4F8BF9;
    border-radius: 0.5rem;
    padding: 1.5rem;
    background-color: rgba(79, 139, 249, 0.1);
    margin-bottom: 1.5rem;
    margin-top: 1rem !important; /* Use !important to ensure margin */
    font-size: 18px;
    color: inherit;
}
/* Target bold text within that specific container */
div[data-testid="stVerticalBlock"] > [style*="flex-direction: column;"] > [data-testid="stVerticalBlock"]:nth-child(2) b {
    color: inherit !important;
}
/* Style for small text above buttons */
.button-prompt { font-size: 0.9rem; color: #a0a0a0; text-align: center; margin-bottom: 0.1rem; }
</style>
...
# --- NEW: Handle Lobby Join Link ---
try:
    query_params = st.query_params
    if query_params and "lobby" in query_params:
        lobby_code = query_params["lobby"]
        st.session_state['join_lobby_id'] = lobby_code
        st.session_state['just_joined_via_link'] = True
        st.query_params.clear()
        # Use HTML redirect hack
        st.markdown(f'<meta http-equiv="refresh" content="0; url=Multiplayer">', unsafe_allow_html=True)
        st.stop()
except AttributeError:
     pass # Older Streamlit version
...
# --- Description text (CSS will style its container) ---
st.markdown("""
...
# --- Conditional Login/Register or Dashboard Button ---
if 'logged_in' in st.session_state and st.session_state.logged_in:
    st.write("You are logged in. Please proceed to your dashboard.")
    st.markdown(f""" <a href="Dashboard" target="_self" class="dashboard-button-link" style="max-width: 400px; margin: 1rem auto;"> Go to My Dashboard 🚀 </a> """, unsafe_allow_html=True)
else:
    ...
    with col1:
        ...
        st.markdown(f""" <a href="Register" target="_self" class="dashboard-button-link"> 📝 Register Now </a> """, unsafe_allow_html=True)
    with col2:
        st.markdown('<p class="button-prompt">Already have an account?</p>', unsafe_allow_html=True)
        st.markdown(f""" <a href="Login" target="_self" class="dashboard-button-link"> 🔐 Login </a> """, unsafe_allow_html=True)